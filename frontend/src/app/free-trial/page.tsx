...
function FreeTrialContent() {
  ...
  const [waCaptured, setWaCaptured] = useState(false);
  const [sessionId] = useState(() => {
    if (typeof window === 'undefined') return '';
    let id = sessionStorage.getItem('_ft_sid');
    if (!id) { id = `${Date.now()}-${Math.random().toString(36).slice(2)}`; sessionStorage.setItem('_ft_sid', id); }
    return id;
  });
  ...
  useEffect(() => {
    setLoading(true);
    setIndex(0); setSelected([]); setRevealed(false); setScore(0); setDone(false); setError(''); setShowMobilePromo(false); setPromoPopupDismissed(false); setShowWaCapture(false);
  ...
  return (
    <div className="min-h-screen bg-gray-50" dir={isAr ? 'rtl' : 'ltr'}>
      ...
      {showWaCapture && (
        <div className="fixed inset-0 z-50 flex items-end justify-center lg:items-center" dir={isAr ? 'rtl' : 'ltr'}>
          <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" />
          <div className="relative w-full max-w-md bg-white rounded-t-3xl lg:rounded-3xl p-6 pb-8 shadow-2xl animate-slide-up">
            <button onClick={() => { setWaCaptured(true); setShowWaCapture(false); }}
              className="absolute top-4 right-4 w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-gray-400 hover:bg-gray-200 transition text-lg">
              ×
            </button>
            <div className="text-center mb-5">
              <div className="text-3xl mb-2">✅</div>
              <h3 className="text-lg font-extrabold text-gray-900 leading-tight">
                {isAr ? '!أحسنت! لقد أجبت على 5 أسئلة' : 'Bien ! Vous avez déjà répondu à 5 questions.'}
              </h3>
              <p className="text-sm text-gray-500 mt-2 leading-relaxed">
                {isAr
                  ? 'لحفظ تقدمك واستلام نتيجتك في نهاية الاختبار، أدخل رقم واتساب الخاص بك'
                  : 'Pour sauvegarder votre progression et recevoir votre score à la fin du test entrez votre numéro WhatsApp :'}
              </p>
            </div>
            <div className="space-y-3">
              <input
                ...
                autoFocus
              />
              <button onClick={submitWa}
                className="w-full py-3.5 rounded-2xl font-bold text-white text-sm hover:opacity-90 transition shadow-lg shadow-violet-300/40 flex items-center justify-center gap-2"
                style={{ background: 'linear-gradient(135deg,#7c3aed,#6366f1)' }}>
                ...
                {isAr ? 'متابعة الاختبار المجاني' : 'Continuer mon essai gratuit'}
              </button>
              <button onClick={() => { setWaCaptured(true); setShowWaCapture(false); }}
        ...
        <div className="lg:hidden fixed inset-0 z-50 flex items-end justify-center" dir={isAr ? 'rtl' : 'ltr'}>
          ...
          <div className="relative w-full max-w-lg bg-white rounded-t-3xl p-6 pb-8 animate-slide-up shadow-2xl">
            ...
            <div className="flex items-center justify-center gap-4 mt-4">
              {[{ name: 'Bankily', src: '/images/bankily.png' }, { name: 'Masrivi', src: '/images/masrivi.png' }, { name: 'Sedad', src: '/images/sedad.png' }].map(({ name, src }) => (
